...
const slashCommandSubcommands1 = require("./SlashCommandSubcommands");
let SlashCommandBuilder = class SlashCommandBuilder {
    ...
    addSubcommandGroup(input) {
        ...
        const hasSubcommands = options.some((item) => item instanceof slashCommandSubcommands1.SlashCommandSubcommandBuilder);
        if (hasSubcommands)
            throw new RangeError(`You cannot mix subcommands and subcommand groups at the root level.`);
        // Get the final result
        const result = typeof input === 'function' ? input(new slashCommandSubcommands1.SlashCommandSubcommandGroupBuilder()) : input;
        Assertions_1.assertReturnOfBuilder(result, slashCommandSubcommands1.SlashCommandSubcommandGroupBuilder);
        // Push it
        options.push(result);
        return this;
    }
    ...
    addSubcommand(input) {
        ...
        Assertions_1.validateMaxOptionsLength(options);
        // Make sure there is no subcommand at the root level - if there is, throw
        const hasSubcommandGroups = options.some((item) => item instanceof slashCommandSubcommands1.SlashCommandSubcommandGroupBuilder);
        if (hasSubcommandGroups)
            throw new RangeError(`You cannot mix subcommands and subcommand groups at the root level.`);
        // Get the final result
        const result = typeof input === 'function' ? input(new slashCommandSubcommands1.SlashCommandSubcommandBuilder()) : input;
        Assertions_1.assertReturnOfBuilder(result, slashCommandSubcommands1.SlashCommandSubcommandBuilder);
        // Push it
        options.push(result);
        return this;
    }
